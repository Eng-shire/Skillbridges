...
class TrackingServiceProvider extends AbstractServiceProvider implements BootableServiceProviderInterface {
	...
	public function provide_tracking_data() {
		...
		$masteriyo_product = isset( $data['product_data'][ $masteriyo_slug ] )
		? $data['product_data'][ $masteriyo_slug ]
		: null;
		...
		if ( ! empty( $data['product_data'] ) && is_array( $data['product_data'] ) ) {
			foreach ( $data['product_data'] as $slug => $prod ) {
				...
				if ( isset( $prod['product_type'] ) && $prod['product_type'] === 'plugin' ) {
		...
		$trimmed = array(
			'base_product'           => $data['base_product'] ?? 'Masteriyo',
			'product_data'           => $masteriyo_product ? array( $masteriyo_slug => $masteriyo_product ) : array(),
			'plugin_activation_date' => $data['plugin_activation_date'] ?? null,
			'publish_course_count'   => $data['publish_course_count'] ?? null,
			'enrolled_users_count'   => $data['enrolled_users_count'] ?? null,
			'masteriyo_install_days' => $data['masteriyo_install_days'] ?? null,
